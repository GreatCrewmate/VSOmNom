...
class PlayState extends MusicBeatState
{
	...
	public static var portgf:Character;
	public static var boyfriend:Boyfriend;
	public static var bfport:Boyfriend;
	...
	public static var deathByCandy:Bool = false; */
	...
	override public function create()
	{
		...
		/* switch(SONG.song.toLowerCase())
		{
			...
			case 'roping':
				...
				//Distraction Warning
				dist_alert = new FlxSprite(460, 300);
				dist_alert.frames = Paths.getSparrowAtlas('omnom/attack/distrWar');
				dist_alert.animation.addByPrefix('distwarning', 'distracting warning', 24, false);		
				dist_alert.antialiasing = true;
				dist_alert.scrollFactor.set();
				dist_alert.updateHitbox();
				dist_alert.scale.x = 1.15;
				dist_alert.scale.y = 1.15;
				...
				case 'fabricbox':
					{
						defaultCamZoom = 0.9;
						curStage = 'fabricbox';
						var fbackground:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('omnom/fbackground'));
						if(FlxG.save.data.antialiasing)
							{
								fbackground.antialiasing = true;
							}
						fbackground.scrollFactor.set(0.9, 0.9);
						fbackground.active = false;
						add(fbackground);

						var ffloor:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('omnom/ffloor'));
						ffloor.setGraphicSize(Std.int(ffloor.width * 1.1));
						ffloor.updateHitbox();
						if(FlxG.save.data.antialiasing)
							{
								ffloor.antialiasing = true;
							}
						ffloor.scrollFactor.set(0.9, 0.9);
						ffloor.active = false;
						add(ffloor);

						var fforeground:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('omnom/fforeground'));
						fforeground.setGraphicSize(Std.int(fforeground.width * 0.9));
						fforeground.updateHitbox();
						if(FlxG.save.data.antialiasing)
							{
								fforeground.antialiasing = true;
							}
						fforeground.scrollFactor.set(1.3, 1.3);
						fforeground.active = false;

						add(fforeground);

						spider = new FlxSprite(1100, -17);
						spider.frames = Paths.getSparrowAtlas('omnom/attack/spider');
						spider.animation.addByPrefix('s1attack', 'spider attack', 24, false);
						spider.animation.addByPrefix('idle', 'spider idle', 24, false);
						if(FlxG.save.data.antialiasing)
							{
								spider.antialiasing = true;
							}
						if (FlxG.save.data.distractions)
						{
							add(spider);
						...
						spiat = new FlxSprite(1100, -17);
						spiat.frames = Paths.getSparrowAtlas('omnom/attack/spider');
						spiat.animation.addByPrefix('s1attack', 'spider attack', 24, false);
						spiat.animation.addByPrefix('idle', 'spider idle', 24, false);
						if(FlxG.save.data.antialiasing)
							{
								spiat.antialiasing = true;
							}
					}
		...
		portgf = new Character(400, 130, curGf);
		portgf.scrollFactor.set(0.95, 0.95);
	...
	function OmNIntro(?dialogueBox:DialogueBox):Void
		{	
								...
								case 2:
								{
									if (daNote.isSustainNote && daNote.wasGoodHit)
									...
									else
										{
											if (daNote.mustPress)
											{
												FlxG.sound.play(Paths.sound('catch_star03'), 1);
												health -= 0.5;
												vocals.volume = 0;
												remove(spider);
												add(spiat);
												spiat.animation.play('s1attack');
												boyfriend.playAnim('unconsin');
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					switch (SONG.song.toLowerCase()) {
						case 'bamboo':
							remove(boyfriend);
							remove(gf);
							bfport = new Boyfriend(770, 450, SONG.player1); //-579,-115
							add(portgf);
							add(bfport);
							bfport.playAnim('portalbf');
							portgf.playAnim('pogrtalf');
				...
				else
				{
					...
					switch (nextSongLowercase) {
						case 'spider':
							LoadingState.loadAndSwitchState(new VideoState("assets/videos/snew/cutscene_third.webm", new PlayState()));
						default:
	...
	private function keyShit():Void // I've invested in emma stocks
	{
		...
		if(SONG.song.toLowerCase() == "bamboo") {
			if(FlxG.keys.justPressed.SPACE) {
				trace('GF portal test');
				gf.playAnim('pogrtalf');
			}
		}
	...
	/* function SPATANIM(pointless:Bool = false):Void //PS: SW
	{
		if(!(SONG.song.toLowerCase() == "star")){
			trace("Cannot use a distraction outside Star.");
		}
		
			new FlxTimer().start(0.1, function(tmr:FlxTimer)
			{			
						spider.animation.play('s1preatt');

					new FlxTimer().start(0.375, function(tmr:FlxTimer)
					{
							spider.animation.play('s1preattid');
					});
			});

			new FlxTimer().start(0.85, function(tmr:FlxTimer)
			{
					spider.animation.play('s1attack');
			});
	}

	function SPATTACK(pointless:Bool = false, candyB:String = 'candy_break'):Void
	{
		if(!(SONG.song.toLowerCase() == "star")){
			trace("Cannot use a distraction outside Star.");
		}
			...
			new FlxTimer().start(0.1, function(tmr:FlxTimer)
			{
				FlxG.sound.play(Paths.sound('catch_star01'), 1);
				dist_alert.animation.play('distwarning');

				
				new FlxTimer().start(0.375, function(tmr:FlxTimer)
				{
					FlxG.sound.play(Paths.sound('catch_star02'), 1);
					dist_alert.animation.play('distwarning');
				});
			});
				...
				new FlxTimer().start(0.33, function(tmr:FlxTimer)
			{
					if(!bfDodging) {
						boyfriend.playAnim('unconsin');
						FlxG.sound.play(Paths.sound(candyB), 1);
						deathByCandy = true;
						health -= 0.5;
					}
				});
